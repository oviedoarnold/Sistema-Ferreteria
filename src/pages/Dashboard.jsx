...
function Dashboard() {
  ...
  /*
    Lo facturado y lo anulado son dos cosas distintas. Las anuladas siguen
    en el historial de abajo, que es actividad, pero no suman en ninguna
    cifra comercial: una venta que se deshizo no es ingreso.
  */
  const ventasVigentes = useMemo(() => sales.filter(esVentaVigente), [sales])

  const stats = useMemo(() => {
    const sumarTotales = (ventas) => ventas.reduce((a, s) => a + Number(s.total || 0), 0)
    const todaySales = sumarTotales(ventasVigentes.filter((s) => esVentaDelDia(s)))
    const monthSales = sumarTotales(ventasVigentes.filter((s) => esVentaDelMes(s)))
    ...
    const receivable = ventasVigentes.reduce((a, s) => a + getSaleBalance(s), 0)
    return { todaySales, monthSales, low, out, receivable }
  }, [products, ventasVigentes])

  const topProducts = useMemo(() => {
    const map = new Map()
    ventasVigentes.forEach((s) => (s.items || s.products || []).forEach((i) => {
